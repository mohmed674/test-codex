...
from .forms import (DepartmentRoleForm, EvaluationCriteriaForm, JobTitleForm,
                    ProductionStageTypeForm, RiskThresholdForm, UnitForm)
...
def _build_admin_dashboard_context():
    ...
    monitoring_data = {
        "discrepancies_count": (
            InventoryDiscrepancy.objects.count() if InventoryDiscrepancy else 0
        ),
        "recent_suspicious_count": (
            SuspiciousActivity.objects.filter(timestamp__gte=last_week).count()
            if SuspiciousActivity
            else 0
        ),
        "preventive_actions_count": (
            AutoPreventiveAction.objects.filter(is_resolved=False).count()
            if AutoPreventiveAction
            else 0
        ),
    }
    ...
    context = {
        ...
        "total_revenue": (
            (SaleOrder.objects.aggregate(Sum("total"))["total__sum"] or 0)
            if SaleOrder
            else 0
        ),
        "total_expense": (
            (
                JournalEntry.objects.filter(entry_type="expense").aggregate(
                    Sum("amount")
                )["amount__sum"]
                or 0
            )
...
def _norm(s: str) -> str:
    ...
    return (
        ...
        .replace("\u0640", "")
        .translate(
            str.maketrans("", "", "".join(chr(c) for c in range(0x064B, 0x0653)))
        )
    )
...
SECTION_OF_KEY = {
    ...
    "vendor_portal": "portals",
    # Inventory & Warehouse
    "inventory": "inventory",
    "warehouse_map": "inventory",
    "shipping": "inventory",
    "products": "inventory",
    # Manufacturing & PLM
    "mrp": "manufacturing",
    ...
    "internal_monitoring": "risk",
    "monitoring": "risk",
    "risk_management": "risk",
    "work_regulations": "risk",
    "legal": "risk",
    "tracking": "risk",
    # Documents & Knowledge
    "document_center": "documents",
    "knowledge_center": "documents",
    "media": "documents",
    "contracts": "documents",
    ...
    "whatsapp_bot": "marketing",
    "notifications": "marketing",
    "voice_commands": "marketing",
    # Analytics & BI
    "bi": "analytics",
    "dashboard_center": "analytics",
    "demand_forecasting": "analytics",
    "ai_decision": "analytics",
    # Integrations & Sync
    "api_gateway": "integrations",
    "offline_sync": "integrations",
    "mobile": "integrations",
...
def launcher_view(request):
    ...
    ctx = {
        ...
        "title": (
            "لوحة التطبيقات" if str(lang).lower().startswith("ar") else "Applications"
        ),
    }
...
def overview_view(request):
    SaleInvoice = safe_get_model("sales", "SaleInvoice") or safe_get_model(
        "sales", "SaleOrder"
    )
    ...
    total_sales = (
        (SaleInvoice.objects.aggregate(Sum("total_amount"))["total_amount__sum"] or 0)
        if SaleInvoice
        else 0
    )
    total_transactions = (
        (CashTxn.objects.aggregate(Sum("amount"))["amount__sum"] or 0) if CashTxn else 0
    )
...
def dashboard(request):
    ...
    return render(
        request, "core/dashboard.html", {"widgets": widgets, "apps": apps_ctx}
    )
...
def admin_dashboard_pdf_view(request):
    ...
    return render_to_pdf_weasy(
        "core/admin_dashboard_pdf.html", context, filename="dashboard_report.pdf"
    )
...
def unit_list(request):
    ...
    headers = _headers(
        ...
        ("نشِطة", "Active"),
        ("وحدة مجمعة", "Bulk Unit"),
    )
    rows: List[TableRow] = []
    for u in units:
        utype = (
            u.get_unit_type_display()
            if hasattr(u, "get_unit_type_display")
            else getattr(u, "unit_type", "—")
        )
        rows.append(
            TableRow(
                cells=[
                    None,
                    getattr(u, "name", str(u)),
...
def criteria_list(request):
    ...
    for c in criteria:
        ctype = (
            c.get_criteria_type_display()
            if hasattr(c, "get_criteria_type_display")
            else getattr(c, "criteria_type", "—")
        )
        rows.append(
            TableRow(
                cells=[
        ...
        rows.append(
            TableRow(
                cells=[
                    ...
                    (
                        getattr(s, "order", None)
                        if getattr(s, "order", None) is not None
                        else "—"
                    ),
                    (
                        getattr(s, "estimated_duration_minutes", None)
                        if getattr(s, "estimated_duration_minutes", None) is not None
                        else "—"
                    ),
...
def risk_threshold_list(request):
    ...
    for r in risks:
        sev = (
            r.get_severity_level_display()
            if hasattr(r, "get_severity_level_display")
            else getattr(r, "severity_level", "—")
        )
        rows.append(
            TableRow(
                cells=[
...
def risk_threshold_update(request, pk):
    ...
    if form.is_valid():
        ...
        return redirect("core:risk_threshold_list")
    return render(
        request, "core/risk_threshold_form.html", {"form": form, "object": obj}
    )
...
def department_role_list(request):
    ...
    return render(
        request,
        "core/department_role_list.html",
...
def department_role_update(request, pk):
    ...
    if form.is_valid():
        ...
        return redirect("core:department_role_list")
    return render(
        request, "core/department_role_form.html", {"form": form, "object": obj}
    )
