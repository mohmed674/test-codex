...
def ensure_line_in_list(src: str, list_name: str, line_to_add: str) -> str:
    """يضيف سطر داخل قائمة مثل: urlpatterns = [ ... ] إن لم يكن موجوداً."""
    m = re.search(rf"{list_name}\s*=\s*\[(.*?)\]", src, flags=re.S)
    if m:
        inside = m.group(1)
        if line_to_add.strip() not in inside:
            new_inside = (
                inside + ("\n" if inside.strip() else "") + line_to_add
            ).rstrip()
            start, end = m.span(1)
...
inv: dict[str, Any] = json.loads(read(INV)) if INV.exists() else {}
apps_info: dict[str, Any] = inv.get("apps", {})
missing_admin: list[str] = inv.get("missing_admin", [])

# ---------- 1) settings sanity (STATIC/MEDIA/Templates dirs) ----------
(core_tpl_dir := BASE / "core" / "templates" / "core").mkdir(
...
if missing_admin:
    ...
    if "from core.models" not in admin_src:
        ...
        if core_miss:
            admin_src = (
                f"from core.models import {', '.join(sorted(set(core_miss)))}\n"
                + admin_src
            )
            for M in sorted(set(core_miss)):
                if (
                    re.search(
                        rf"@admin\.register\({M}\)|admin\.site\.register\({M}\)",
                        admin_src,
                    )
                    is None
                ):
    ...
    for app_dir in sorted([p for p in apps_root.iterdir() if p.is_dir()]):
        ...
        if (app_dir / "urls.py").exists():
            cfg = ensure_line_in_list(
                cfg,
                "urlpatterns",
                f"    path('{app_dir.name}/', include('apps.{app_dir.name}.urls')),",
            )
...
st: dict[str, Any] = {
    "last_script": "erp_autofix_all.py",
    "status": "autofixed",
...
try:
    ...
    # تجنب إيقاف السكربت بسبب JSON تالف
    pass
write(STATE, json.dumps(st, indent=2, ensure_ascii=False))

print(
    "✅ ERP autofix completed: admin/core, inventory proxy, suppliers urls/api, config urls wiring, core/templates/base.html"
)
