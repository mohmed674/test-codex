...
from .models import SaleInvoice, SalesPerformance, SmartSalesSuggestion
...
def sales_dashboard_view(request):
    ...
    invoices_qs = SaleInvoice.objects.select_related("client", "client__partner").all()
    ...
    top_clients = (
        SaleInvoice.objects.values("client__partner__name")  # ← الاسم الصحيح
...
def invoice_list_view(request):
    invoices = SaleInvoice.objects.select_related("client", "client__partner").all()

    if request.GET.get("download_pdf") == "1":
        return render_to_pdf(
            "sales/invoice_pdf.html", {"invoices": invoices, "request": request}
        )
