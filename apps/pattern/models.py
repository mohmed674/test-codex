...
class PatternDesign(models.Model):
    ...
    image = models.ImageField(
        upload_to="patterns/images/", blank=True, null=True, verbose_name="صورة"
    )
    gerber_file = models.FileField(
        upload_to="patterns/gerber/", blank=True, null=True, verbose_name="ملف Gerber"
    )
    pattern_type = models.CharField(
        max_length=20, choices=PatternType.choices, default=PatternType.BASIC
    )
...
class PatternPiece(models.Model):
    design = models.ForeignKey(
        PatternDesign, on_delete=models.CASCADE, related_name="pieces"
    )
...
class PatternExecution(models.Model):
    production_order = models.ForeignKey(
        ProductionOrder, on_delete=models.CASCADE, related_name="executed_patterns"
    )
