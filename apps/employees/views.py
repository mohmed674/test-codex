...
def _pick(
    names: Iterable[str], candidates: Sequence[str], default: Optional[str] = None
) -> Optional[str]:
    s = set(names)
    for c in candidates:
        if c in s:
...
def _emp_name_field() -> str:
    return (
        _pick(
            _model_fields(Employee),
...
def _find_fk_to(
    model: type, target_model: type, fallbacks: Sequence[str] = ("employee", "user")
) -> Optional[str]:
...
def _reverse_or_fallback(names: Sequence[str], *args: Any, **kwargs: Any) -> str:
    for n in names:
...
class EmployeeCreateView(CreateView):
    ...
    def get_success_url(self) -> str:
        return _reverse_or_fallback(
            ("employees:employee_list", "employee_list", "core:overview")
        )
...
def employee_profile(request, pk):
    ...
    disc_emp_fk = _find_fk_to(
        DisciplineRecord, Employee, fallbacks=("employee", "emp", "worker")
    )
    ...
    if date_from:
        if (from_date := _safe_parse_date(date_from)) is not None:
            records = records.filter(**{f"{disc_date_f}__gte": from_date})
    if date_to:
        if (to_date := _safe_parse_date(date_to)) is not None:
    ...
    if regulation:
        ...
        if fk_emp:
            ...
            if emp_user:
                q &= Q(**{fk_user: emp_user})
        regulation_status = EmployeeAgreement.objects.filter(q).first()

    first_of_month = date.today().replace(day=1)
    incentive = (
        MonthlyIncentive.objects.filter(employee=emp, month=first_of_month).first()
        if "employee" in _model_fields(MonthlyIncentive)
        and "month" in _model_fields(MonthlyIncentive)
        else None
    )
    ...
    if request.GET.get("export") == "1":
        ...
        for r in records:
            ...
            r_created_by = (
                getattr(r, "created_by", "-") if hasattr(r, "created_by") else "-"
            )
            r_date_val = getattr(r, disc_date_f, None)
            if isinstance(r_date_val, (date, datetime)):
                r_date = r_date_val.strftime("%Y-%m-%d")
            else:
                r_date = str(r_date_val or "-")
            ws.append(
                [
                    r_type,
                    str(r_value) if r_value is not None else "-",
                    r_reason,
                    str(r_created_by),
                    r_date,
                ]
        ...
        safe_name = str(getattr(emp, _emp_name_field(), f"emp_{emp.pk}"))
        response["Content-Disposition"] = (
            f"attachment; filename=discipline_{safe_name}.xlsx"
        )
        wb.save(response)
        return response
    ...
    if request.GET.get("download_pdf") == "1":
        ...
        try:
            html_string = render_to_string(
                "employees/employee_discipline_pdf.html", context
            )
...
def calculate_all_salaries(request):
    ...
    for emp in employees:
        ...
        Salary.objects.update_or_create(
            employee=emp, month=month, defaults={"amount": final}
        )
