...
class PaymentOrder(models.Model):
    ...
    payment_method = models.CharField(
        "طريقة الدفع", max_length=20, choices=PAYMENT_METHODS
    )
    ...
    created_by = models.ForeignKey(
        User, on_delete=models.SET_NULL, null=True, verbose_name="أنشئ بواسطة"
    )

    def __str__(self) -> str:
        return f"أمر دفع #{self.pk} - {self.supplier.name}"
...
class Asset(models.Model):
    ...
    asset_type = models.CharField(
        "نوع الأصل", max_length=50, choices=ASSET_TYPES, db_index=True
    )
    ...
    def __str__(self) -> str:
        # نحصل على اسم النوع المعروض بدون استخدام type: ignore
        display_getter = getattr(self, "get_asset_type_display", None)
        display = display_getter() if callable(display_getter) else str(self.asset_type)
        return f"{self.name} - {display}"

    def annual_depreciation(self) -> Decimal:
        """القيمة السنوية للإهلاك."""
        return self.value * (self.depreciation_rate / Decimal("100"))
...
class Expense(models.Model):
    ...
    category = models.CharField(
        "التصنيف", max_length=50, choices=CATEGORY_CHOICES, db_index=True
    )
    amount = models.DecimalField(
        "المبلغ",
        max_digits=10,
...
class Invoice(models.Model):
    ...
    payment_method = models.CharField(
        "طريقة الدفع", max_length=20, choices=PAYMENT_METHOD_CHOICES
    )
    sale_type = models.CharField(
        "نوع البيع", max_length=20, choices=SALE_TYPE_CHOICES, default="cash"
    )
    ...
    status = models.CharField(
        "الحالة", max_length=20, choices=STATUS_CHOICES, default="unpaid", db_index=True
    )
    notes = models.TextField("ملاحظات", blank=True, null=True)
    created_at = models.DateTimeField("تاريخ الإنشاء", auto_now_add=True)
...
class Payment(models.Model):
    ...
    class Meta:
        ...
        ordering = ["-payment_date"]
        # فهرس لمساعدة الاستعلامات على التاريخ
        indexes = [
            models.Index(fields=["payment_date"]),
        ]
...
class Account(models.Model):
    ...
    type = models.CharField(
        "نوع الحساب", max_length=50, choices=ACCOUNT_TYPES, db_index=True
    )
...
class JournalItem(models.Model):
    ...
    account = models.ForeignKey(
        Account, on_delete=models.CASCADE, verbose_name="الحساب"
    )
    debit = models.DecimalField(
        "مدين",
        max_digits=12,
...
class AccountingSuggestionLog(models.Model):
    ...
    suggested_by_ai = models.BooleanField(
        "مُقترح بواسطة الذكاء الاصطناعي؟", default=True
    )
...
class BillOfMaterial(models.Model):
    ...
    @components.setter
    def components(self, value: Dict[str, Any]) -> None:
        """تعيين المكوّنات من قاموس وحفظه JSON."""
...
class ManufacturingOrder(models.Model):
    ...
    order_number = models.CharField(
        "رقم أمر التصنيع", max_length=50, unique=True, db_index=True
    )
    ...
    quantity = models.PositiveIntegerField(
        "الكمية", default=1, validators=[MinValueValidator(1)]
    )
    status = models.CharField(
        "الحالة",
        max_length=20,
    ...
    actual_cost = models.DecimalField(
        "التكلفة الفعلية", max_digits=12, decimal_places=2, blank=True, null=True
    )
    notes = models.TextField("ملاحظات", blank=True, null=True)
