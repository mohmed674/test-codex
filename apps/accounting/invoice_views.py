# apps/accounting/invoice_views.py
...
from .forms import InvoiceItemForm, PurchaseInvoiceForm, SalesInvoiceForm
from .models import InvoiceItem, PurchaseInvoice, SalesInvoice
...
def _generate_invoice_number(model_cls, prefix: str) -> str:
    ...
    last = (
        model_cls.objects.filter(number__startswith=base)
        ...
        if form.is_valid() and formset.is_valid():
            ...
            for f in formset.forms:
                if not getattr(f, "cleaned_data", None) or f.cleaned_data.get("DELETE"):
                    continue
                item: InvoiceItem = f.save(commit=False)
                item.sales_invoice = invoice
                item.purchase_invoice = None
                item.save()
                total += Decimal(item.quantity or 0) * Decimal(item.unit_price or 0)
            ...
            return redirect("accounting:sales_invoice_detail", invoice.pk)
...
@login_required
def sales_invoice_detail(request, pk: int):
...
def create_purchase_invoice(request):
    ...
    if request.method == "POST":
        ...
        if form.is_valid() and formset.is_valid():
            ...
            return redirect("accounting:purchase_invoice_detail", invoice.pk)
        messages.error(request, "⚠️ تأكد من تعبئة الحقول بشكل صحيح.")
    else:
...
@login_required
def purchase_invoice_detail(request, pk: int):
...
# ============== طباعة PDF ==============
@login_required
def print_invoice_pdf(request, invoice_type: str, pk: int):
    if invoice_type == "sales":
        invoice = get_object_or_404(SalesInvoice, pk=pk)
        template_name = "accounting/invoices/sales_invoice_pdf.html"
        items_qs = InvoiceItem.objects.filter(sales_invoice=invoice).order_by("id")
    else:
        invoice = get_object_or_404(PurchaseInvoice, pk=pk)
        template_name = "accounting/invoices/purchase_invoice_pdf.html"
        items_qs = InvoiceItem.objects.filter(purchase_invoice=invoice).order_by("id")

    html = get_template(template_name).render({"invoice": invoice, "items": items_qs})
...
def sales_invoice_excel(request):
    invoices = (
        SalesInvoice.objects.select_related("customer").all().order_by("-date_issued")
    )
        ...
        rows.append(
            {
                ...
                # get_status_display موجودة تلقائيًا من choices
                "الحالة": inv.get_status_display(),  # type: ignore[attr-defined]
                "المبلغ": float(inv.total_amount or 0),
            }
        )

    return export_to_excel(rows, filename="sales_invoices.xlsx")
