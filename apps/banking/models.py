...
class BankProvider(models.Model):
    ...
    config = models.JSONField(
        default=dict, blank=True, verbose_name=_("إعدادات إضافية")
    )
...
class BankAccount(models.Model):
    ...
    external_id = models.CharField(
        max_length=120, blank=True, null=True, verbose_name=_("المعرف الخارجي")
    )
    ...
    class Meta:
        ...
        indexes = [
            models.Index(
                ...
                name="ix_banking_acc_provider_accno",
            ),
...
class Payment(models.Model):
    ...
    direction = models.CharField(
        max_length=3, choices=PaymentDirection.choices, verbose_name=_("الاتجاه")
    )
    method = models.CharField(
        max_length=20, choices=PaymentMethod.choices, verbose_name=_("الطريقة")
    )
    ...
    amount = models.DecimalField(
        max_digits=14, decimal_places=2, verbose_name=_("المبلغ")
    )
    currency = models.CharField(
        max_length=3,
        choices=Currency.choices,
    ...
    reference = models.CharField(
        max_length=120, unique=True, verbose_name=_("مرجع العملية")
    )
    description = models.CharField(
        max_length=255, blank=True, null=True, verbose_name=_("الوصف")
    )
    ...
    created_by = models.ForeignKey(
        User, on_delete=models.SET_NULL, null=True, blank=True, verbose_name=_("أنشأها")
    )
    created_at = models.DateTimeField(
        auto_now_add=True, verbose_name=_("تاريخ الإنشاء")
    )
    ...
    def __str__(self) -> str:
        return (
            f"{self.reference} — {self.amount} {self.currency} "
            f"({self.get_status_display()})"  # type: ignore[attr-defined]
        )
...
class Transfer(models.Model):
    ...
    source_account = models.ForeignKey(
        ...
        verbose_name=_("من حساب"),
    )
    destination_account = models.ForeignKey(
        BankAccount,
        on_delete=models.PROTECT,
        related_name="incoming_transfers",
        verbose_name=_("إلى حساب"),
    )
    amount = models.DecimalField(
        max_digits=14, decimal_places=2, verbose_name=_("المبلغ")
    )
    ...
    scheduled_at = models.DateTimeField(
        blank=True, null=True, verbose_name=_("موعد مجدول")
    )
    executed_at = models.DateTimeField(
        blank=True, null=True, verbose_name=_("تاريخ التنفيذ")
    )
    reference = models.CharField(
        max_length=120, unique=True, verbose_name=_("مرجع التحويل")
    )
    ...
    def __str__(self) -> str:
        return (
            f"Transfer {self.reference} — {self.amount} {self.currency} "
            f"({self.get_status_display()})"  # type: ignore[attr-defined]
        )
...
class BankTransaction(models.Model):
    ...
    type = models.CharField(
        max_length=6, choices=TransactionType.choices, verbose_name=_("النوع")
    )
    amount = models.DecimalField(
        max_digits=14, decimal_places=2, verbose_name=_("المبلغ")
    )
    ...
    value_date = models.DateTimeField(
        default=timezone.now, verbose_name=_("تاريخ القيمة")
    )
    booked_at = models.DateTimeField(
        default=timezone.now, verbose_name=_("تاريخ القيد")
    )
...
class ReconciliationBatch(models.Model):
    ...
    matched_count = models.PositiveIntegerField(
        default=0, verbose_name=_("عدد المطابق")
    )
    unmatched_count = models.PositiveIntegerField(
        default=0, verbose_name=_("غير مطابق")
    )
    notes = models.TextField(blank=True, null=True, verbose_name=_("ملاحظات"))
    created_by = models.ForeignKey(
        User, on_delete=models.SET_NULL, null=True, blank=True, verbose_name=_("أنشأها")
    ...
    class Meta:
        ...
        indexes = [
            ...
            models.Index(
                ...
                name="ix_banking_recon_period",
            ),
        ]

    def __str__(self) -> str:
        return (
            f"Recon {self.account.name} "
...
class WebhookEvent(models.Model):
    ...
    signature = models.CharField(
        max_length=255, blank=True, null=True, verbose_name=_("التوقيع")
    )
    payload = models.JSONField(verbose_name=_("البيانات"))
    received_at = models.DateTimeField(
        auto_now_add=True, verbose_name=_("وقت الاستلام")
    )
    processed_at = models.DateTimeField(
        blank=True, null=True, verbose_name=_("وقت المعالجة")
    )
    status = models.CharField(
        max_length=10,
    ...
    def __str__(self) -> str:
        return (
            f"{self.provider.code}:{self.event_type} "
            f"[{self.get_status_display()}]"  # type: ignore[attr-defined]
        )
