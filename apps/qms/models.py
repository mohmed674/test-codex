...
class MeasurementType(models.TextChoices):
    ...
    ATTRIBUTE = "attribute", _("وصفي/سِمي (Attribute)")
...
class QualityCharacteristic(TimeStampedModel):
    ...
    measurement_type = models.CharField(
        ...
        default=MeasurementType.VARIABLE,
    )
    ...
    target = models.DecimalField(
        _("القيمة المستهدفة"), max_digits=14, decimal_places=6, null=True, blank=True
    )
    lsl = models.DecimalField(
        _("الحد الأدنى المسموح (LSL)"),
        max_digits=14,
        decimal_places=6,
        null=True,
        blank=True,
    ...
    decimal_places = models.PositiveSmallIntegerField(
        _("عدد المنازل العشرية"), default=2
    )
    ...
    class Meta:
        ...
        indexes = [models.Index(fields=["key"]), models.Index(fields=["is_active"])]
...
class DefectType(TimeStampedModel):
    ...
    severity = models.CharField(
        _("الخطورة"),
        max_length=16,
        choices=SeverityClass.choices,
        default=SeverityClass.MAJOR,
    )
    ...
    def __str__(self) -> str:
        try:
            sev_label = SeverityClass(self.severity).label
        except Exception:
            sev_label = self.severity
        return f"{self.code} — {self.name} ({sev_label})"
...
class AQLPlan(TimeStampedModel):
    ...
    standard = models.CharField(
        _("المعيار"), max_length=64, default="ISO 2859-1 / MIL-STD-105E"
    )
    stage = models.CharField(
        _("مرحلة الفحص"),
        max_length=16,
        choices=InspectionStage.choices,
        default=InspectionStage.INCOMING,
    )
    level = models.CharField(
        _("مستوى الفحص"),
        max_length=4,
        choices=InspectionLevel.choices,
        default=InspectionLevel.GII,
    )
    aql = models.DecimalField(
        _("قيمة AQL"),
        max_digits=5,
        decimal_places=2,
        help_text=_("مثال: 0.65 ، 1.0 ، 2.5"),
        db_index=True,
    )
    ...
    class Meta:
        ...
        indexes = [models.Index(fields=["aql", "level", "mode", "is_active"])]
    ...
    class Meta:
        ...
        constraints = [
            models.UniqueConstraint(
                fields=["plan", "lot_size_from", "lot_size_to", "code_letter"],
                name="uq_aql_codeletter_range",
            )
        ]
        indexes = [models.Index(fields=["plan", "code_letter"])]
...
class AQLSamplingRow(TimeStampedModel):
    ...
    class Meta:
        ...
        constraints = [
            models.UniqueConstraint(
                fields=["plan", "code_letter", "sample_size", "accept", "reject"],
                name="uq_aql_sampling_row",
            )
        ]
        indexes = [models.Index(fields=["plan", "code_letter"])]
...
class InspectionLot(TimeStampedModel):
    ...
    # Generic link to source doc
    source_ct = models.ForeignKey(
    ...
    source_ref = models.CharField(
        _("مرجع المستند"), max_length=128, null=True, blank=True
    )
    ...
    accept_number = models.PositiveIntegerField(
        _("عدد القبول (Ac)"), null=True, blank=True
    )
    reject_number = models.PositiveIntegerField(
        _("عدد الرفض (Re)"), null=True, blank=True
    )
    ...
    inspector = models.ForeignKey(
        ...
        blank=True,
        on_delete=models.SET_NULL,
        related_name="qms_inspections",
        verbose_name=_("المفتش"),
    )
    ...
    def __str__(self) -> str:
        try:
            stage_label = InspectionStage(self.stage).label
        except Exception:
            stage_label = self.stage
        return f"{self.code} — {stage_label}"

    @property
    def defects_total(self) -> int:
        # حساب إجمالي العيوب دون الاعتماد على self.results (لتفادي مشاكل typing)
        total = (
            InspectionResult.objects.filter(lot=self)
            .aggregate(
                total=Sum(
                    F("defects_critical") + F("defects_major") + F("defects_minor")
                )
            )
            .get("total")
            or 0
...
class InspectionResult(TimeStampedModel):
    ...
    value = models.DecimalField(
        _("قيمة القياس"), max_digits=14, decimal_places=6, null=True, blank=True
    )
    ...
    class Meta:
        ...
        indexes = [models.Index(fields=["is_pass"])]

    @property
    def defects_total(self) -> int:
        return (
            (self.defects_critical or 0)
            + (self.defects_major or 0)
            + (self.defects_minor or 0)
        )
...
class Nonconformity(TimeStampedModel):
    ...
    defect_type = models.ForeignKey(
        DefectType, on_delete=models.PROTECT, verbose_name=_("نوع العيب")
    )
    ...
    class Meta:
        ...
        indexes = [models.Index(fields=["defect_type"])]
...
class ControlChart(TimeStampedModel):
    ...
    chart_type = models.CharField(
        ...
        default=ChartType.XBAR_R,
    )
    subgroup_size = models.PositiveIntegerField(
        _("حجم المجموعة الفرعية (n)"), default=5
    )
    rule_set = models.CharField(
        _("قواعد الكشف"),
        max_length=16,
        choices=RuleSet.choices,
        default=RuleSet.BASIC_3SIGMA,
    )
    cl = models.DecimalField(
        _("CL"), max_digits=14, decimal_places=6, null=True, blank=True
    )
    ucl = models.DecimalField(
        _("UCL"), max_digits=14, decimal_places=6, null=True, blank=True
    )
    lcl = models.DecimalField(
        _("LCL"), max_digits=14, decimal_places=6, null=True, blank=True
    )
    ...
    class Meta:
        ...
        indexes = [models.Index(fields=["chart_type", "is_active"])]

    def __str__(self) -> str:
        try:
            ct_label = ChartType(self.chart_type).label
        except Exception:
            ct_label = self.chart_type
        return f"{self.process} [{ct_label}]"
...
class Subgroup(TimeStampedModel):
    ...
    mean = models.DecimalField(
        _("المتوسط (X̄)"), max_digits=14, decimal_places=6, null=True, blank=True
    )
    r = models.DecimalField(
        _("المدى (R)"), max_digits=14, decimal_places=6, null=True, blank=True
    )
    s = models.DecimalField(
        _("الانحراف المعياري (S)"),
        max_digits=14,
        decimal_places=6,
        null=True,
        blank=True,
    ...
    class Meta:
        ...
        constraints = [
            models.UniqueConstraint(
                fields=["chart", "number"], name="uq_spc_chart_subgroup_number"
            )
        ]
...
class DataPoint(TimeStampedModel):
    subgroup = models.ForeignKey(
        ...
        on_delete=models.CASCADE,
        related_name="points",
        verbose_name=_("المجموعة الفرعية"),
    )
    value = models.DecimalField(
        _("القيمة"), max_digits=14, decimal_places=6, null=True, blank=True
    )
    defects = models.PositiveIntegerField(_("عدد العيوب"), default=0)
    sample_size = models.PositiveIntegerField(
        _("حجم العينة للخصائص (Attribute)"), null=True, blank=True
    )
    ...
    class Meta:
        ...
        indexes = [models.Index(fields=["is_out_of_control"])]

    def __str__(self) -> str:
        return f"Point #{self.pk} — Subgroup {self.subgroup.pk}"
...
class CapabilityStudy(TimeStampedModel):
    process = models.ForeignKey(
        ControlProcess,
        on_delete=models.CASCADE,
        related_name="capability_studies",
        verbose_name=_("العملية"),
    )
    characteristic = models.ForeignKey(
        QualityCharacteristic,
    ...
    lsl = models.DecimalField(
        _("LSL"), max_digits=14, decimal_places=6, null=True, blank=True
    )
    usl = models.DecimalField(
        _("USL"), max_digits=14, decimal_places=6, null=True, blank=True
    )
    target = models.DecimalField(
        _("Target"), max_digits=14, decimal_places=6, null=True, blank=True
    )

    cp = models.DecimalField(
        _("Cp"), max_digits=10, decimal_places=4, null=True, blank=True
    )
    cpk = models.DecimalField(
        _("Cpk"), max_digits=10, decimal_places=4, null=True, blank=True
    )
    pp = models.DecimalField(
        _("Pp"), max_digits=10, decimal_places=4, null=True, blank=True
    )
    ppk = models.DecimalField(
        _("Ppk"), max_digits=10, decimal_places=4, null=True, blank=True
    )
...
class CAPARecord(TimeStampedModel):
    ...
    capa_type = models.CharField(
        _("النوع"), max_length=3, choices=CAPAType.choices, default=CAPAType.CORRECTIVE
    )
    status = models.CharField(
        _("الحالة"), max_length=16, choices=CAPAStatus.choices, default=CAPAStatus.OPEN
    )
    ...
    effectiveness_verified_at = models.DateTimeField(
        _("تاريخ التحقق من الفاعلية"), null=True, blank=True
    )
    effectiveness_verified_by = models.ForeignKey(
        ...
        blank=True,
        on_delete=models.SET_NULL,
        related_name="capa_verified",
        verbose_name=_("مُتحقق"),
    )
...
class CAPAAction(TimeStampedModel):
    ...
    verification_result = models.CharField(
        _("نتيجة التحقق"), max_length=128, null=True, blank=True
    )
    ...
    def __str__(self) -> str:
        try:
            label = CAPAType(self.action_type).label
        except Exception:
            label = self.action_type
        return f"{self.record.code} — {label}"
...
def _generate_code(prefix: str) -> str:
    ...
    if prefix == "QMS-INSP":
        ...
        last = (
            CAPARecord.objects.filter(code__startswith=base)
            .order_by("-code")
            .values_list("code", flat=True)
            .first()
        )
...
def qms_inspectionlot_presave(
    sender: type[InspectionLot], instance: InspectionLot, **kwargs: Any
) -> None:
...
def qms_capa_presave(
    sender: type[CAPARecord], instance: CAPARecord, **kwargs: Any
) -> None:
    if not instance.code:
