...
def discipline_report(request):
    ...
    if request.GET.get("export") == "1":
        ...
        ws.append(
            ["اسم الموظف", "نوع الإجراء", "القيمة", "السبب", "أنشئ بواسطة", "التاريخ"]
        )
    ...
    if request.GET.get("download_pdf") == "1":
        html_string = render_to_string(
            "discipline/discipline_report.html",
    ...
    return render(request, "discipline/discipline_report.html", {"records": records})
...
def manager_discipline_report(request):
    ...
    if request.GET.get("export") == "1":
        ...
        response["Content-Disposition"] = (
            "attachment; filename=manager_discipline_report.xlsx"
        )
        wb.save(response)
        return response
    ...
    if request.GET.get("download_pdf") == "1":
        html_string = render_to_string(
            "discipline/manager_discipline_report.html",
            {"records": records, "request": request},
        )
        html = HTML(string=html_string)
        result = html.write_pdf()
        response = HttpResponse(content_type="application/pdf")
        response["Content-Disposition"] = (
            "inline; filename=manager_discipline_report.pdf"
        )
        response.write(result)
        return response

    return render(
        request, "discipline/manager_discipline_report.html", {"records": records}
    )
