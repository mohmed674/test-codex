...
class DisciplineRecord(models.Model):
    ...
    employee = models.ForeignKey(
        Employee, on_delete=models.CASCADE, verbose_name="الموظف"
    )
    date = models.DateField(auto_now_add=True, verbose_name="تاريخ الإجراء")
    type = models.CharField(
        max_length=50, choices=TypeChoices.choices, verbose_name="نوع الإجراء"
    )
    value_type = models.CharField(
        max_length=10,
        choices=ValueTypeChoices.choices,
        default=ValueTypeChoices.AMOUNT,
        verbose_name="نوع القيمة",
    )
    value = models.DecimalField(
        max_digits=10, decimal_places=2, null=True, blank=True, verbose_name="القيمة"
    )
    reason = models.TextField(verbose_name="السبب")
    created_by = models.CharField(max_length=100, verbose_name="تم بواسطة")
    notes = models.TextField(blank=True, null=True, verbose_name="ملاحظات")
    related_month = models.CharField(
        max_length=7, blank=True, null=True, verbose_name="الشهر المرتبط"
    )
