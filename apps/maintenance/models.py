...
class MaintenanceRequest(models.Model):
    machine = models.ForeignKey(
        Machine, on_delete=models.CASCADE, verbose_name="الماكينة"
    )
    reported_by = models.ForeignKey(
        Employee, on_delete=models.SET_NULL, null=True, verbose_name="المبلغ"
    )
    ...
    status = models.CharField(
        max_length=20, choices=REQUEST_STATUS, default="pending", verbose_name="الحالة"
    )
    resolution_notes = models.TextField(
        blank=True, null=True, verbose_name="ملاحظات التنفيذ"
    )
