...
def main_dashboard(request):
    ...
    total_transactions = (
        CashTransaction.objects.aggregate(total=Sum("amount"))["total"] or 0
    )
    ...
    current_month_sales = (
        SaleInvoice.objects.filter(date__gte=start_of_month).aggregate(
    ...
    growth_rate = (
        round(((current_month_sales - last_month_sales) / last_month_sales) * 100, 2)
...
def app_home(request):
    return render(
        request, "apps/dashboard_center/home.html", {"app": "dashboard_center"}
    )
