...
from .models import Answer, Survey, SurveyResponse
            ...
            for question in survey.surveyquestion_set.all():
                ...
                if question.question_type == "text":
                    Answer.objects.create(
                        response=response, question=question, answer_text=answer_value
                    )
                elif question.question_type == "multiple_choice":
                    choice = question.surveychoice_set.get(id=answer_value)
                    Answer.objects.create(
                        response=response, question=question, selected_choice=choice
                    )
...
def survey_results(request, survey_id):
    ...
    answers = Answer.objects.filter(response__survey=survey).select_related(
...
def export_survey_pdf(request, survey_id):
    ...
    html = get_template("survey/survey_results_pdf.html").render(
        {"survey": survey, "results_by_question": results_by_question}
    )
        ...
        data.append(
            {
                ...
                "الاختيار": (
                    ans.selected_choice.choice_text if ans.selected_choice else ""
                ),
