...
class ProductTracking(models.Model):
    ...
    tracking_code = models.CharField(
        max_length=100, unique=True, verbose_name="كود التتبع"
    )
    customer_name = models.CharField(max_length=255, verbose_name="اسم العميل")
    payment_method = models.CharField(
        max_length=20, choices=PAYMENT_METHOD_CHOICES, verbose_name="طريقة الدفع"
    )
    ...
    shipping_company = models.CharField(
        max_length=100, blank=True, verbose_name="شركة الشحن"
    )
...
class ProductTrackingMovement(models.Model):
    ...
    status = models.CharField(
        max_length=20, choices=ProductTracking.STATUS_CHOICES, verbose_name="الحالة"
    )
