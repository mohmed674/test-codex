...
def advanced_tracking_report_view(request):
    ...
    records = records.annotate(
        delivery_duration=ExpressionWrapper(
            F("delivery_date") - F("shipment_date"), output_field=fields.DurationField()
        )
    )
...
def tracking_customer_view(request):
    ...
    context = {"tracking": tracking_info, "movements": movements, "code": tracking_code}
...
def tracking_api_view(request):
    ...
    try:
        ...
        movements = ProductTrackingMovement.objects.filter(tracking=tracking).order_by(
            "-timestamp"
        )
...
def advanced_tracking_report(request):
    ...
    if delay_threshold:
        records = records.filter(
            delivery_date__isnull=False, shipment_date__isnull=False
        ).extra(
            where=["delivery_date - shipment_date > interval %s day"],
            params=[delay_threshold],
        )

    context = {
        "records": records,
        "filters": {"company": company, "city": city, "delay": delay_threshold},
    }
    return render(request, "tracking/advanced_report.html", context)
