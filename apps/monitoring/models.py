...
class DistributionOrder(models.Model):
    ...
    status = models.CharField(
        "الحالة", max_length=20, choices=ORDER_STATUS_CHOICES, default="Pending"
    )
...
class Shipment(models.Model):
    ...
    order = models.ForeignKey(
        DistributionOrder, on_delete=models.CASCADE, verbose_name="الطلب"
    )
...
# ===================================================
# ✅ نموذج الالتقاط المطلوب من الإشعارات (CapturedImage)
# ===================================================


class CapturedImage(models.Model):
    camera_id = models.CharField("معرّف الكاميرا", max_length=100, blank=True, null=True)
    timestamp = models.DateTimeField("وقت الالتقاط", default=timezone.now)
    captured_by = models.ForeignKey(
        ...
        null=True,
        blank=True,
        related_name="monitoring_captures",
    )
    camera_user = models.CharField("مستخدم الكاميرا", max_length=150, blank=True)
    image = models.ImageField(
        "الصورة",
        upload_to="monitoring/captures/",
        blank=True,
        null=True,
    )
    notes = models.TextField("ملاحظات", blank=True)
    ...
    def __str__(self) -> str:
        label = self.camera_id or "كاميرا غير معروفة"
        dt = self.timestamp.strftime("%Y-%m-%d %H:%M") if self.timestamp else ""
        return f"{label} @ {dt}"

    class Meta:
        ordering = ["-timestamp"]
        verbose_name = "صورة ملتقطة"
        verbose_name_plural = "صور ملتقطة"
