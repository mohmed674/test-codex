...
class ProductionOrder(models.Model):
    ...
    product = models.ForeignKey(
        FinishedProduct, on_delete=models.CASCADE
    )  # ← هنا التعديل
...
class MaterialConsumption(models.Model):
    order = models.ForeignKey(
        ProductionOrder, on_delete=models.CASCADE, related_name="materials_used"
    )
...
class BillOfMaterials(models.Model):
    product = models.ForeignKey(
        FinishedProduct, on_delete=models.CASCADE, verbose_name="المنتج النهائي"
    )
...
class QualityCheck(models.Model):
    product = models.ForeignKey(
        FinishedProduct, on_delete=models.CASCADE, verbose_name="المنتج"
    )  # ← هنا التعديل
    order = models.ForeignKey(
        ProductionOrder, on_delete=models.CASCADE, verbose_name="أمر التشغيل"
    )
    stage = models.ForeignKey(
        ProductionStage, on_delete=models.CASCADE, verbose_name="مرحلة الإنتاج"
    )
...
class ProductVersion(models.Model):
    ...
    season = models.CharField(
        max_length=100, blank=True, null=True, verbose_name="الموسم (صيف - شتاء...)"
    )
    ...
    change_notes = models.TextField(
        blank=True, null=True, verbose_name="ملاحظات التعديل"
    )
...
class ProductionLog(models.Model):
    ...
    order = models.ForeignKey(
        ProductionOrder, on_delete=models.CASCADE, null=True, blank=True
    )
    stage = models.ForeignKey(
