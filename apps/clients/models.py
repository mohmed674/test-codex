...
class Partner(models.Model):
    ...
    type = models.CharField(
        ...
        default=PartnerType.PERSON,
    )

    phone = models.CharField(
        _("رقم الهاتف"), max_length=30, blank=True, null=True, unique=False
    )
    email = models.EmailField(_("البريد الإلكتروني"), blank=True, null=True)
    tax_number = models.CharField(
        _("الرقم الضريبي"), max_length=64, blank=True, null=True
    )
    accounting_code = models.CharField(
        _("كود الحساب المحاسبي"), max_length=64, blank=True, null=True
    )
...
class Address(models.Model):
    ...
    postal_code = models.CharField(
        _("الرمز البريدي"), max_length=20, blank=True, null=True
    )
    ...
    def address_type_label(self) -> str:
        """Static-checker-friendly replacement for get_address_type_display()."""
        mapping: Dict[str, str] = dict(self.AddressType.choices)
        return mapping.get(self.address_type, self.address_type)

    def __str__(self) -> str:
        return f"{self.partner.name} - {self.address_type_label()}"
...
class Customer(models.Model):
    ...
    profile_image = models.ImageField(
        _("صورة العميل"), upload_to="clients/", blank=True, null=True
    )
