...
class ProductVersion(models.Model):
    ...
    product = models.ForeignKey(
        ProductTemplate, on_delete=models.CASCADE, related_name="versions"
    )
    ...
    documents = models.ManyToManyField(
        PLMDocument, blank=True, related_name="product_versions"
    )
    created_at = models.DateTimeField(auto_now_add=True)
...
class ProductLifecycle(models.Model):
    ...
    product = models.ForeignKey(
        ProductTemplate, on_delete=models.CASCADE, related_name="lifecycles"
    )
...
class Bom(models.Model):
    ...
    product_version = models.ForeignKey(
        ProductVersion, on_delete=models.CASCADE, related_name="boms"
    )
...
class BomLine(models.Model):
    ...
    component = models.ForeignKey(
        ProductTemplate, on_delete=models.PROTECT, related_name="as_component_in_boms"
    )
    ...
    wastage_pct = models.DecimalField(  # نسبة الهالك
        max_digits=5,
        decimal_places=2,
        default=Decimal("0"),
    )
    operation = models.CharField(  # مرحلة الاستخدام (قص/خياطة...)
        max_length=120,
        blank=True,
        null=True,
    )
...
class ChangeRequest(models.Model):
    ...
    product = models.ForeignKey(
        ProductTemplate, on_delete=models.CASCADE, related_name="change_requests"
    )
    from_version = models.ForeignKey(
        ProductVersion,
        on_delete=models.PROTECT,
        related_name="cr_from",
        blank=True,
        null=True,
    )
    ...
    change_type = models.CharField(
        max_length=20, choices=TYPE_CHOICES, default="design"
    )
    ...
    attachments = models.ManyToManyField(
        PLMDocument, blank=True, related_name="change_requests"
    )
    created_at = models.DateTimeField(auto_now_add=True)
