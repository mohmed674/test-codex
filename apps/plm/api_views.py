...
from .serializers import (BomLineSerializer, BomSerializer,
                          ChangeRequestItemSerializer, ChangeRequestSerializer,
                          LifecycleStageSerializer, PLMDocumentSerializer,
                          ProductLifecycleSerializer,
                          ProductTemplateSerializer, ProductVersionSerializer)
...
class ProductVersionViewSet(BaseViewSet):
    queryset = (
        ProductVersion.objects.select_related("product")
...
class BomViewSet(BaseViewSet):
    queryset = (
        Bom.objects.select_related("product_version", "product_version__product")
    ...
    search_fields = (
        "code",
        "product_version__version_code",
        "product_version__product__code",
...
class ChangeRequestViewSet(BaseViewSet):
    queryset = (
        ChangeRequest.objects.select_related("product", "from_version", "to_version")
        .prefetch_related("attachments", "items")
        .all()
    )
...
class ChangeRequestItemViewSet(BaseViewSet):
    queryset = ChangeRequestItem.objects.select_related(
        "change_request", "bom", "bom_line"
    ).all()
    serializer_class = ChangeRequestItemSerializer
    search_fields = (
        "change_request__number",
        "bom__code",
        "bom_line__component__code",
        "bom_line__component__name",
    )
    ordering_fields = ("id",)
    ordering = ("id",)
