...
class Supplier(models.Model):
    ...
    governorate = models.CharField(
        max_length=100, blank=True, null=True, verbose_name="المحافظة"
    )
    supplier_type = models.CharField(
        max_length=100, blank=True, null=True, verbose_name="نوع المورد"
    )
    ...
    created_by = models.ForeignKey(
        Employee, on_delete=models.SET_NULL, null=True, verbose_name="أنشأ بواسطة"
    )
    ...
    def total_payments(self):
        return (
            SupplierInvoice.objects.filter(supplier=self, status="paid").aggregate(
