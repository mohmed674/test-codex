...
def _fallback_list(request):
    return JsonResponse(
        {"ok": True, "endpoint": "suppliers/api/list", "using": "fallback"}, status=200
    )


def _fallback_detail(request, pk: int):
    return JsonResponse(
        {"ok": True, "endpoint": f"suppliers/api/detail/{pk}", "using": "fallback"},
        status=200,
    )
...
if HAVE_API_VIEWS and hasattr(_api, "SupplierListAPIView"):
    urlpatterns.append(
        path("list/", _api.SupplierListAPIView.as_view(), name="supplier_list_api")
    )
else:
...
if HAVE_API_VIEWS and hasattr(_api, "SupplierDetailAPIView"):
    urlpatterns.append(
        path(
            "detail/<int:pk>/",
            _api.SupplierDetailAPIView.as_view(),
            name="supplier_detail_api",
        )
    )
else:
    urlpatterns.append(
        path("detail/<int:pk>/", _fallback_detail, name="supplier_detail_api")
    )
